...
To provide a complete beginner's understanding of the data flow overhead across the 7 phases:
*   **Packet Size 1 (Scraping):** Playwright extracts up to `~150KB - 200KB` of raw dehydrated JSON text per single Mutual Fund page.
*   **Packet Size 2 (Chunking):** Phase 3 takes the ~150KB array and slices it mathematically into ~25 tightly packed chunks exactly **500 characters** in length with **50-character** overlaps to maintain sentence cohesion.
*   **Packet Size 3 (Vectors):** Phase 4 converts these exact strings into `384` floating-point numbers per chunk, totaling a minuscule `~1.5KB` memory fingerprint per math array inside ChromaDB.
...
| :--- | :--- | :--- | :--- |
| **Playwright (Python)** | **Native** | High (Modern architecture) | **Selected:** Groww is a React SPA; basic scrapers fail to read the dynamic context. Playwright physically renders the hidden JSON (`__NEXT_DATA__`) flawlessly. |
| **BeautifulSoup4** | None | Low | **Rejected:** Cannot execute JavaScript; returns empty div skeletons on SPAs preventing accurate AUM retrieval. |
| **Selenium** | Native | Moderate | **Rejected:** Slower execution speed and higher resource overhead compared to Playwright. |

### 1.5 System Design & Backend Logic Specifications
To circumvent visual layout changes on the Groww website, the scraper does not look for CSS classes. Instead, it locates the massive `<script id="__NEXT_DATA__">` block appended by Next.js servers. 
By pulling out this pure JSON Object, the backend backend logically sweeps for the string `"Cash Equivalent"` explicitly—bypassing the fact that it is a hidden sub-array of "Debt". It guarantees that the pie-chart weights are 100% matched in our scraped text packet payload.
...
### 4.4 Why X over Y? (Preference Selection Criteria)
**Vector Database Consideration:**
| Evaluated Options | Cloud API Required? | Local File Persistence | Verdict |
| :--- | :--- | :--- | :--- |
| **ChromaDB** | **No** | **Excellent (SQLite based)** | **Selected:** Incredibly lightweight, requires 0 setup, and persists data directly to a local folder, making it completely free with instant `0ms` querying. |
...
| :--- | :--- | :--- | :--- | :--- |
| **HuggingFace BGE via FastEmbed (bge-small-en-v1.5)** | 384 | Ultra-Fast | **Top-tier for small models** | **Selected:** Runs the BGE model mathematically via ONNX. It drops the PyTorch memory footprint from ~500MB+ down to ~150MB, easily clearing Render Cloud's strict Free Tier 512MB RAM constraints while keeping perfect 384D mathematical accuracy. |
| **HuggingFace BGE via PyTorch (sentence-transformers)** | 384 | Fast | Excellent | **Rejected:** PyTorch dependencies consistently triggered Render Out-of-Memory (OOM) fatal crashes inside `512MB` instances during DB loading. |
...
### 6.4 Why X over Y? (Preference Selection Criteria)
| Evaluated Options | Free Tier Speed | Instruction Adherence Focus | Verdict |
| :--- | :--- | :--- | :--- |
| **Groq (Llama 3.1 8B)** | **Ultra-Fast (Hardware LPU)** | **Very High** | **Selected:** The fastest open-source LLM inference API available (~800 tokens per second). It generates the 3-sentence restriction flawlessly. Perfect for quick iterative RAG chats without keeping users waiting. |
| **Gemini 1.5 Flash** | Slower (Software based) | Very High | **Rejected:** Free tier was occasionally timing out or adding multi-second latency spikes across API nodes. |

### 6.5 Performance Optimizations & Logic (Singletons)
...
*   **Latency Drop:** This optimization completely eliminated the 8 to 12-second cold-boot loading times on Render, ensuring semantic retrieval latency is purely limited to `0.3s` RAM lookups.
*   **Conversational Suppression:** A core backend rule instructs the LLM to secretly inject a `<NO_SOURCES>` flag for generic conversational queries like `"Hi"`. The FastAPI backend physically strips this packet marker, allowing the UI to cleanly refuse to render citation web-links for non-factual chats.
